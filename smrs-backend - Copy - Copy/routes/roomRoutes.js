...
// PUT update room by buildingRoom
router.put('/rooms/:buildingRoom', authenticate, async (req, res) => {
  try {
    const buildingRoom = req.params.buildingRoom;
    const { status } = req.body;
    console.log('Gửi yêu cầu cập nhật phòng:', { buildingRoom, status });

    const room = await Room.findOne({ buildingRoom });
    if (!room) {
      console.error('Phòng không tồn tại:', buildingRoom);
      return res.status(404).json({ message: 'Phòng không tồn tại' });
    }

    if (room.status !== 'Trống' && status === 'Đầy') {
      console.warn('Phòng không ở trạng thái Trống:', room);
      return res.status(400).json({ message: 'Chỉ có thể đặt phòng ở trạng thái Trống' });
    }
